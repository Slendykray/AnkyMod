...
namespace AnkyMod.Modules {
    internal static class Language
    {
        ...
        private static void Language_collectLanguageRootFolders(List<string> obj) {
            string path = Path.Combine(Path.GetDirectoryName(AnkyPlugin.instance.Info.Location), "Language");
        ...
        public static void PrintOutput(string fileName = "") {
            ...
            if (!string.IsNullOrEmpty(fileName)) {
                string path = Path.Combine(Directory.GetParent(AnkyPlugin.instance.Info.Location).FullName, "Language", "en", fileName);
